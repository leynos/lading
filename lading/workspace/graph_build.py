...
from lading.workspace._coercion import (
    _expect_mapping,
    _expect_sequence,
    _expect_string,
...
def build_workspace_graph(
    ...
    metadata : collections.abc.Mapping[str, typing.Any]
        Parsed ``cargo metadata`` with ``workspace_root`` (path string),
        ``packages`` (package mappings), and ``workspace_members`` (id
        strings).

    Returns
    -------
    WorkspaceGraph
        The crates and dependency relationships derived from ``metadata``.

    Raises
    ------
    WorkspaceModelError
        When ``workspace_root`` is missing, the ``packages``/
        ``workspace_members`` sequences are malformed, or a member is unknown.
    """
    try:
