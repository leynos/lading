...
def _invoke_cargo_metadata_command(
    command_runner: CommandRunner,
    root_path: Path | None,
) -> tuple[int, str, str]:
    """Invoke ``cargo metadata`` and map spawn errors to domain exceptions."""
    try:
...
def _parse_cargo_metadata_output(stdout_text: str) -> cabc.Mapping[str, typ.Any]:
...
def load_cargo_metadata(
    ...
    *,
    ...
    exit_code, stdout_text, stderr_text = _invoke_cargo_metadata_command(
        command_runner, root_path
    )
    if exit_code != 0:
        raise CargoMetadataInvocationError(exit_code, stdout_text, stderr_text)
    return _parse_cargo_metadata_output(stdout_text)
