...
preservation, formatter errors, argument validation, and the tracked-file
discovery contract.
"""
...
from scripts.tests.markdown_format_test_support import (
    CHECKER,
...
def _recorded_calls(call_log: Path) -> list[list[str]]:
    """Return the formatter invocations recorded by the fixture."""
    return [
...
def test_rejects_large_noncanonical_source_without_a_broken_pipe(
    formatter: tuple[Path, Path],
    tmp_path: Path,
) -> None:
    """Report format drift without leaking a SIGPIPE diagnostic."""
    executable, call_log = formatter
