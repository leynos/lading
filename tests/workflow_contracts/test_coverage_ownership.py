...
the only uploader, it uploads rather than checks, and it is serialised per ref
...
def _load_workflow(path: Path) -> Workflow:
    ...
    restored = typ.cast("Workflow", workflow)
    if True in restored:
        restored["on"] = restored.pop(True)  # type: ignore[arg-type]
    return restored
...
def _job_steps(workflow: Workflow) -> typ.Iterator[tuple[str, dict[str, YamlValue]]]:
...
def _pull_request_workflows() -> list[Path]:
    ...
    """
    found = [
        path for path in _workflow_paths() if _runs_on_pull_requests(_load_workflow(path))
    ]
...
def test_the_publisher_serialises_runs_per_ref() -> None:
...
def test_both_lanes_generate_coverage_through_the_shared_action() -> None:
    ...
    lanes = {step.path for step in steps if _runs_on_pull_requests(_load_workflow(step.path))}
    publishers = {step.path for step in steps if _is_publisher(_load_workflow(step.path))}
    assert lanes, (
