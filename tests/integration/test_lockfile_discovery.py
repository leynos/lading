...
def _git(repo: Path, *args: str) -> None:
    ...
    # ruff: ignore[subprocess-without-shell-equals-true] - fixed argv, test fixture setup
    subprocess.run(
...
def test_discovery_returns_tracked_lockfiles_with_manifests(tmp_path: Path) -> None:
    ...
    assert set(result) == {root_lock, nested_lock}, (
        f"discovery should return exactly the tracked lockfiles with adjacent "
        f"manifests; got {sorted(result)}"
    )
    assert target_lock not in result, (
        f"lockfiles under target/ must be excluded; got {sorted(result)}"
    )
...
def test_discovery_ignores_untracked_lockfiles(tmp_path: Path) -> None:
    ...
    assert tracked in result, f"tracked lockfile missing from {sorted(result)}"
    assert untracked not in result, (
        f"untracked lockfile must not be discovered; got {sorted(result)}"
    )
