...
def test_discover_tracked_lockfiles_raises_for_non_git_directory(
    ...
    *,
    ...
    assert excinfo.value.workspace_root == tmp_path, (
...
@pytest.mark.parametrize(
    ...
    [
        ("fatal: not a git repository", "not_git"),
        ("fatal: bad revision", "git_error"),
    ],
)
def test_discovery_failures_record_bounded_outcome(
    tmp_path: Path, stderr: str, expected_reason: str
) -> None:
    """Each discovery failure increments the bounded failure counter once."""
    ...
    with pytest.raises(lockfile.LockfileDiscoveryError):
        lockfile.discover_tracked_lockfiles(tmp_path, runner)

    assert (
        metrics.counter_value(lockfile.DISCOVERY_FAILURE_METRIC, reason=expected_reason)
        == 1
    ), f"discovery failure should be counted under reason={expected_reason!r}"
