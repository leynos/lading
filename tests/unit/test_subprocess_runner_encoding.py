...
def test_relay_stream_keeps_binary_mode_after_encoding_fallback() -> None:
...
def test_invoke_via_subprocess_preserves_unicode_with_narrow_stdout(
    ...
    payload = "Cargo metadata: ś ń\n"
    child_script = f"import sys; sys.stdout.buffer.write({payload.encode('utf-8')!r})"
    sink = _Cp1252Sink()
...
@given(payload=_UNICODE_TEXT)
def test_relay_stream_preserves_unicode_across_binary_transition(payload: str) -> None:
